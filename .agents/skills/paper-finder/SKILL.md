...
In brief: expand the user's vague memory into 5–10 search terms, run **one** call to `paperhub_utils/paper_search.py` (`--terms ... --top 15 --detail 5`), and present the ranked digest — 5 detailed cards with `[[label]]` wikilinks plus a brief tail. Never Read candidate files wholesale; the script's digest is the context. Read-only: this skill never modifies paper folders or tags.
