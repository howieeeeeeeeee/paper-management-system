...
- The script searches all `.md` files under `paperhub_utils/config/config.json`'s `obsidian.vault_abs_path` when valid; otherwise it falls back to the PaperHub root. It skips any path with a dot-prefixed segment such as `.claude/`, `.agents/`, `.obsidian/`, `.git/`, or `.venv/`.
...
- **Search scope:** visible `.md` files only; dot-prefixed folders are skipped by the script.
