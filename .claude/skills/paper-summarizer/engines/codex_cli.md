...
- The default PaperHub Codex model/reasoning pair is configured in `paperhub_utils/misc/config.json` as `codex_cli_model` and `codex_cli_reasoning_effort`, exported as `config.CODEX_CLI_MODEL` and `config.CODEX_CLI_REASONING_EFFORT`.
- Default pair: `gpt-5.5` + `high` reasoning.
...
1. Call `paper_summarizer.py --prepare-cli-input --external-cli-engine codex-cli` to prepare the prompt/PDF and resolve the selected Codex model/reasoning pair.
...
4. Call `paper_summarizer.py --from-response --external-cli-engine codex-cli`; the script validates Codex artifacts, extracts the sentinel-delimited response block, and organizes the paper.
...
|---|---|---|---|
| `full` | `paper_summarizer.py --prepare-cli-input --external-cli-engine codex-cli --summary-mode full` | `paper_summarizer.py --from-response --external-cli-engine codex-cli --summary-mode full` | metadata note + `ai_summary.md` |
| `metadata-only` | `paper_summarizer.py --prepare-cli-input --external-cli-engine codex-cli --summary-mode metadata-only` | `paper_summarizer.py --from-response --external-cli-engine codex-cli --summary-mode metadata-only` | metadata note only; no `ai_summary.md` |
| `enrich` | `enrich.py --engine codex-cli --prepare-cli-input --folder FOLDER` | `enrich.py --engine codex-cli --from-response --folder FOLDER` | existing folder patched and/or `ai_summary.md` written |
...
Add `--codex-model "gpt-5.5"` or `--codex-reasoning-effort "xhigh"` only when the user explicitly requests a different allowed model/thinking pair. If unspecified, the default is `gpt-5.5` + `high`.
...
- Set the default model in `paperhub_utils/misc/config.json` with `codex_cli_model`.
...
```yaml
---
model: gpt-5.5 (high reasoning) (Codex CLI)
