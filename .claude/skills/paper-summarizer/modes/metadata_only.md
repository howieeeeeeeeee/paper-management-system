...
Engines that support `metadata-only`: **OpenRouter** (`engines/openrouter.md`), **Gemini CLI** (`engines/gemini_cli.md`), **Coding Agent** (`engines/coding_agent.md` — cheapest mode for this engine; reads only the first `METADATA_ONLY_PAGE_LIMIT` pages, no quota gate).
...
Always include token usage when available. For Gemini CLI, `cost: N/A`. For Coding Agent, no token info.
