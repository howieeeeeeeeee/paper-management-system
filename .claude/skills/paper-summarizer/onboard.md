...
1. Find the paper-library root: nearest parent containing `paperhub_utils/config.py`, `.claude/skills/paper-summarizer/SKILL.md`, `SamplePaperBoard.base`, or `onboarding_questionnaire.md`.
2. If `onboarding_questionnaire.md` exists at the root, read it before asking anything.
   ...
   - The questionnaire must never contain a real API key. If it appears to include one, tell the user to rotate it, remove it from the file, and use `paperhub_utils/.env` instead.
...
4. Read `paperhub_utils/misc/onboarding.json` as the resumable progress ledger and `paperhub_utils/misc/config.json` as the user-editable runtime config.
...
11. If a step changes a real project setting, update `paperhub_utils/misc/config.json` too. For example, Git preference belongs in JSON as `use_git`; `paperhub_utils/config.py` exposes it as `USE_GIT` for Python code.
...
2. Resolve the Obsidian vault root for base setup:
   ...
   - Otherwise use `paperhub_utils/misc/config.json` if valid.
...
- **Root path**: if the discovered root conflicts with `config.py`, ask whether to use the detected root or the configured root.
- **Git**:
...
- **OpenRouter API key**:
  - If the questionnaire says OpenRouter is desired, verify that `OPENROUTER_API_KEY` is available from the environment or `paperhub_utils/.env`.
  - Verify without printing the key:

    ```bash
    uv run python -c "import os; from dotenv import load_dotenv; load_dotenv('.env'); print(bool(os.getenv('OPENROUTER_API_KEY')))"
    ```

  - If the key is missing, ask the user to edit `paperhub_utils/.env` and add exactly:
...
- **AI engine**:
  ...
  - Gemini CLI is acceptable when the questionnaire selects it and `gemini` is available.
...
- **Metadata page depth**:
  ...
  - Persist the integer to `paperhub_utils/misc/config.json` as `metadata_only_page_limit`.
...
After the tag flow finishes, update the `initialize_tag_system` step in `paperhub_utils/misc/onboarding.json`.

## 4. Capture research interests

1. Prefer the questionnaire's "Research Interests" section.
2. If it is filled, overwrite `MY_RESEARCH_INTERESTS` in `paperhub_utils/config.py` with that text as a triple-quoted string, preserving newlines.
...
5. Verify the change from the utilities directory:
   ...
   ```bash
   uv run python -c "import config; print(repr(config.MY_RESEARCH_INTERESTS)[:200])"
   ```

6. Update `paperhub_utils/misc/onboarding.json`: mark `configure_research_interests` as `done` or `skipped`, set `completed_at`, and record a one-line `notes` summary such as `set custom`, `kept existing`, or `skipped, left empty`.

## 5. Configure paper label format

The paper-label spec lives in `paperhub_utils/prompt/shared/paper_label.txt` and controls how AI summaries name new paper folders.
...
2. Whether OpenRouter, Gemini CLI, or current-agent processing is available.
...
6. A short done/remaining summary from `paperhub_utils/misc/onboarding.json`.
...
Onboarding is complete when config paths are valid, `use_git` is explicit in `paperhub_utils/misc/config.json`, the tag registry exists, base files are configured or explicitly skipped, and the `uv` environment is installed in `paperhub_utils/`.
