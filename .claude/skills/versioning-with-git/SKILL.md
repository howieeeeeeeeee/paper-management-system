...
description: Version the paper library with git WITHOUT keeping a .git inside the (iCloud-synced) vault. Copies the whole vault into a separate out-of-iCloud "git backup folder" that IS a git repo, and commits all changes there (optionally pulling/pushing a remote). Triggers when the user wants to "version / commit / back up / snapshot my library", "sync to git/GitHub", or after paper-organizer finishes a batch and needs to commit. NOT for summarizing/organizing papers (that is paper-organizer).
---
