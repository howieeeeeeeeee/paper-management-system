...
# Shell variable name -> prepared-result key, per external CLI engine.
# Every listed key is required: emit_shell_config refuses to write a partial
# file, so a caller that sources it can never reach the CLI with an empty flag.
_SHELL_EXPORT_KEYS = {
    "codex-cli": {
        "PROMPT_PATH": "prompt_path",
        "PDF_PATH": "pdf_for_ai_codex_path",
        "ORIGINAL_PDF": "original_pdf_path",
        "PAPERHUB_ROOT": "paperhub_root",
        "MODEL": "codex_cli_model",
        "EFFORT": "codex_cli_reasoning_effort",
    ...
    "agy-cli": {
        "PROMPT_PATH": "prompt_path",
        "PDF_PATH": "pdf_for_ai_agy_path",
        "ORIGINAL_PDF": "original_pdf_path",
        "PAPERHUB_ROOT": "paperhub_root",
        "MODEL": "agy_cli_model",
        "MODEL_LABEL": "agy_model_label",
        "SUMMARY_MODE": "summary_mode",
        "CLEANUP_DIR": "cleanup_dir",
    },
}
...
def emit_shell_config(result: dict, out_path: Path, engine: str) -> None:
    """Write prepared CLI fields as a sourceable shell file.

    Hand-extracting these fields in shell is the failure mode this exists to
    remove: a bad path or renamed key yields an empty string, and the CLI is
    then invoked with an empty flag. `local var=$(...)` masks the exit code
    even under `set -e`, so the caller cannot reliably guard it. Validating
    here and refusing to write a partial file makes that impossible.
    """
    keys = _SHELL_EXPORT_KEYS.get(engine)
    if keys is None:
        raise ValueError(f"--emit-shell does not support engine: {engine}")
    ...
    if missing:
        raise ValueError(
            "Refusing to write shell config; missing/empty required fields: "
            + ", ".join(missing)
        )

    lines = [
        "# Generated by paper_organizer --emit-shell. Source, do not edit.",
        f"# engine={engine}",
    ]
    lines += [f"{var}={shlex.quote(str(result[key]))}" for var, key in keys.items()]
    out_path.write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def main():
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "With --prepare-cli-input, also write a sourceable shell file of the "
            "resolved fields. Fails if any required field is empty, so sourcing it "
            "guarantees the CLI is never invoked with an empty flag."
        ),
    )
    parser.add_argument(
        "--cleanup-cli-input",
    ...
    if args.prepare_cli_input:
        ...
        if args.emit_shell and result.get("success"):
            try:
                emit_shell_config(
                    result,
                    Path(args.emit_shell),
                    args.external_cli_engine or "agy-cli",
                )
            except (ValueError, OSError) as e:
                result = {
                    "success": False,
                    "pdf_path": str(Path(pdf_path_str).resolve()),
                    "error": f"Failed to write shell config: {type(e).__name__}: {e}",
                    "error_type": type(e).__name__,
                }
